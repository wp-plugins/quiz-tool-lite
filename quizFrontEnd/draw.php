...
function drawQuiz($quizID)
{
	...
	else
	{
		...
		$questionArray = generateQuizQuestions($potQuestionArray, $quizOptionsArray);	
		...
		if($timeLimitCheck=="on" && ($timeLimitMinutes>=1 || $timeLimitSeconds >=1))
		{
			...
			echo showCountdown($timeLimitMinutes, $timeLimitSeconds);
			echo '</div>';
...
function drawMarkedQuestion($questionID, $response="", $showFeedback="yes")
{
	...
	if($qType=="text")
	{
		...
		foreach ($optionsRS as $myOptions)
		{
			$optionValue = strtolower(utils::convertTextFromDB($myOptions['optionValue']));
