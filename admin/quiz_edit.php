...
if(isset($_GET['action']))
{
	...
	if($action=="quizEdit")
	{
		$feedback =  '<span class="successText">Quiz updated</span>';
		$quizID= quizEdit();
	}
}
...
?>
    ...
    <div id="quizOverview">
        ...
        <input type="radio" name="questionListType" id="potQuestions"  value="pot" onclick="javascript:toggleLayerVis('customListDiv'); toggleLayerVis('questionPotsDiv');"  <?php if ($quizOptionsArray['questionListType']=='pot'){echo 'checked'; }?>/>
        <label for="potQuestions">Add questions from pots</label><br />
        <input type="radio" name="questionListType" id="customQuestions"  value="custom"  onclick="javascript:toggleLayerVis('customListDiv'); toggleLayerVis('questionPotsDiv');" <?php if ($quizOptionsArray['questionListType']=='custom'){echo 'checked'; }?>/>
		...
		{
			$questionInfo = getQuestionInfo($thisQuestionID);
		...
		if(count($questionErrorArray)>=1 || count($questionTypeErrorArray)>=1)
		{
			...
			if(count($questionTypeErrorArray>=1))
			{
